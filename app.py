"""
Streamlit-интерфейс для демонстрации поискового индекса.

Ответственный: Артём.

Назначение:
Этот файл отвечает только за веб-интерфейс.

Он не реализует поиск сам.
Вместо этого он вызывает C-бинарник ./app:
...
Важно:
Если C-программа в режиме --json напечатает что-то кроме JSON,
frontend не сможет распарсить результат.
"""
...
if search_clicked:
    if not query.strip():
        ...
        with st.spinner("Поиск..."):
            t0 = time.monotonic()
            # Важно:
            # stdout должен содержать только JSON.
            # stderr можно использовать для сообщений об ошибках.
            proc = subprocess.run(
